...
var ioFuncs = map[string]LGFunction{
	"close":       ioClose,
	"create_dir":  ioCreateDir,
	"create_file": ioCreateFile,
	"flush":       ioFlush,
	"file_size":   ioFileSize,
	"lines":       ioLines,
	"input":       ioInput,
	"is_dir":      ioIsDir,
	"is_file":     ioIsFile,
...
func ioIsDir(L *LState) int {
	...
	L.Push(LBool(fi.IsDir()))
	return 1
}

func ioIsFile(L *LState) int {
	path := L.CheckString(1)
	fi, err := os.Stat(path)
	if err != nil {
		L.Push(LFalse)
	...
	L.Push(LBool(fi.Mode().IsRegular()))
	return 1
}
...
func ioCreateFile(L *LState) int {
	path := L.CheckString(1)
	data := L.OptString(2, "")
...
func ioFileSize(L *LState) int {
	fi, err := os.Stat(L.CheckString(1))
	if err != nil {
		L.Push(LNil)
		L.Push(LString(err.Error()))
		return 2
	}
	L.Push(LNumber(fi.Size()))
	return 1
}
