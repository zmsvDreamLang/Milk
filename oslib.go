...
func osChmod(L *LState) int {
	err := os.Chmod(L.CheckString(1), os.FileMode(L.CheckInt(2)))
	if err != nil {
		L.Push(LNil)
...
func osChown(L *LState) int {
	err := os.Chown(L.CheckString(1), L.CheckInt(2), L.CheckInt(3))
	if err != nil {
		L.Push(LNil)
		L.Push(LString(err.Error()))
		return 2
...
func osGetTimeZone(L *LState) int {
	_, offset := time.Now().Zone()
	name, _ := time.Now().Zone()
	L.Push(LString(name))
	L.Push(LNumber(offset))
	return 2
}

func osGetOs(L *LState) int {
	goos := runtime.GOOS
	arch := runtime.GOARCH
	L.Push(LString(goos))
	L.Push(LString(arch))
...
func osGetHomeDir(L *LState) int {
