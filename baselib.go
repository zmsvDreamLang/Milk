...
func baseShow(L *LState) int {
	top := L.GetTop()
	for i := 1; i <= top; i++ {
		toshow := L.Get(i)
		if toshow == LNil {
			fmt.Print("nil")
		} else if tbl, ok := toshow.(*LTable); ok {
			result, err := tableToStringRecursive(L, tbl, "", make(map[*LTable]bool))
			result = "table: \n" + result
			if err != nil {
				fmt.Print("error")
			} else {
		...
		} else if num, ok := toshow.(LNumber); ok {
			fmt.Print(fmt.Sprintf("number: %f", num))
		} else if str, ok := toshow.(LString); ok {
			fmt.Print(fmt.Sprintf("string: %s", str))
		} else if boolean, ok := toshow.(LBool); ok {
			fmt.Print(fmt.Sprintf("bool: %t", boolean))
		} else if fun, ok := toshow.(*LFunction); ok {
			fmt.Print(fmt.Sprintf("%s", fun.String()))
		} else if user, ok := toshow.(*LUserData); ok {
			fmt.Print(fmt.Sprintf("%s", user.String()))
		} else if thread, ok := toshow.(*LState); ok {
			fmt.Print(fmt.Sprintf("%s", thread.String()))
		} else if chn, ok := toshow.(*LChannel); ok {
			fmt.Print(fmt.Sprintf("%s", chn.String()))
		} else {
			fmt.Print(fmt.Sprintf("%s", toshow.String()))
		}
		if i != top {
			fmt.Print("\n")
		}
